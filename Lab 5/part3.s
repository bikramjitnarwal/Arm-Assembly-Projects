...
_start:   	LDR 	R3, =0xFF200020		//R3 holds HEX address
			...
			LDR 	R12, =50000000		// Will give 0.25 seconds for counter
			STR 	R12, [R11]
			MOV 	R12, #0b011
 ...
 */
DO_DELAY: 	//PUSH 	{R7} 	
			LDR 	R12, =0xFFFEC60C	// read timer status
			LDR		R12, [R12]
			CMP 	R12, #0				// check if done counting (when interrupt = 1)
			BEQ 	DO_DELAY
...
RESET: 			MOV 	R6, #0b1111
				...
				MVN 	R10, R10	  // Complement current value of R10// flip EC value
				CMP 	R10, #0
				BEQ 	DO_NOTHING
				B 		DISPLAY

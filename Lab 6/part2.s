/*
 ...
 Bikramjit Narwal, 1005242300
 
The main program below executes an endless loop writing the value of the global variable COUNT to the red lights LEDR. In the interrupt service routine for the Interval Timer the user is to increment the variable COUNT by the value of the RUN global variable, which should be either 1 or 0. The user also is to toggle the value of the RUN global variable in the interrupt service routine for the KEYs, each time a KEY is pressed. When RUN = 0, the main program will display a static count on the red lights, and when RUN = 1, the count shown on the red lights will increment every 0.25 seconds.
